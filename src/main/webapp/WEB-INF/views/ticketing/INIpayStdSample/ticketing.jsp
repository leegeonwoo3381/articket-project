...
<%
	...
	*/
	...
	/* 기타 */
	String siteDomain = "http://localhost:8080/"; //가맹점 도메인 입력
...
<head>
  ...
  <!-- 테스트 JS(샘플에 제공된 테스트 MID 전용) -->
	...
	<link href="${path}https://fonts.googleapis.com/css?family=Roboto:300,400&display=swap" rel="stylesheet">

    <link href="${path}https://fonts.googleapis.com/css?family=Poppins:300,400,500&display=swap" rel="stylesheet">

    
    <link rel="stylesheet" href="${path}resources/fonts/icomoon/style.css">

    <link rel="stylesheet" href="${path}resources/css/ticketing/rome.css">
    
    
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="${path}resources/css/ticketing/bootstrap.min.css">
    
    <!-- Style -->
    <link rel="stylesheet" href="${path}resources/css/ticketing/style.css">
...
<div class="row">
  <h2 class="col-md-12 text-center">티켓 예매</h2>
...
<!--------------------------------공연정보---------------------------------->
  <div class="content col-md-4">
      ...
      <div id="data">
       <div>
        <img id ="poster" style="width:300px; height:400px;" src="https://kopis.or.kr/upload/pfmPoster/PF_PF121749_160202_133331.jpg"><br> 
      	<span id ="cate"></span><br>   <!-- main.js 예매API 정보 받아온곳 -->
        <span id ="prfnm"></span><br>
        <span id ="area"></span><br>
        <span id ="prfplcnm"></span><br>
        <span id ="prfpd"></span><br>
        <input type="text" id ="date_ticketed"><br>
        <input type="number" value="0" id ="coupon_id"><br>
        <input type="number" value="0" id ="check_used"><br>
...
<div class="content col-md-4">
      ...
      <div class="row justify-content-center">
        <div class="col-md-10">
          ...
          <input type="text" class="form-control w-25 mx-auto mb-3 data-input" id="result" placeholder="날짜를 선택해주세요" disabled="">
      ...
      <br>
      	...
      	<h3 style="height:30px; text-align:center;">선택한 날짜 : <span id ="date"></span></h3>	<!-- 달력API 정보 받아온곳 -->
...
<div class="movie-container">
  <label class="movie-font">좌석등급을 선택하세요:</label>
  <select id="movie">
    <option value="69000">R석 (69,000 원)</option>
    <option value="59000">S석 (59,000 원)</option>
    <option value="49000">A석(49,000 원)</option>
    <option value="39000">B석(39,000 원)</option>
    <option value="29000">C석(29,000 원)</option>
    <option value="19000">Standing(19,000 원)</option>
  </select>
</div>
...
<div class="m_container">
  ...
  <div class="m_row">
    <input type="button" class="seat" id="seat_num"  value="H3">
    <div class="seat"></div>
    <div class="seat"></div>
    <div class="seat"></div>
    <div class="seat"></div>
...
<p class="movie-font">
  ...
  <br>
   <p class="movie-font">
   좌석 정보 : <span id="resultSeat"></span>
   <br>
   좌석 수 :  <span id="count">0</span>
    <br>
    지불 금액 : <span id="total">0</span> 원
    </p>    
    
    <input id="user_id" type="number" value="${id}" >
    
<button onclick="INIStdPay.pay('SendPayForm_id')" class="payment-btn" id="sendData">결제요청</button>
<button class="payment-btn" id="testdata">데이터 넣기</button>
	...
	<div style="padding:10px;background-color:#f3f3f3;width:100%;font-size:13px;color: #ffffff;background-color: #000000;text-align: center; display:none"  >
		이니시스 표준결제 결제요청 샘플
	</div>
	
	<table width="650" border="0" cellspacing="0" cellpadding="0" style="padding:10px;" align="center">
		<tr>
			<td bgcolor="6095BC" align="center" style="padding:10px; display:none; ">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF" style="padding:20px">
					...
					<!-- <tr>
						<td > -->
							<!-- 결제요청 -->
							<!-- <button onclick="INIStdPay.pay('SendPayForm_id')" style="">결제요청</button> -->
					<!-- 	</td>
					</tr> -->
					<tr style="display:none;">
						<td>
									...
									<td style="text-align:left;">
										<form id="SendPayForm_id" name="" method="POST" >
											...
											<div style="border:2px #dddddd double;padding:10px;background-color:#f3f3f3;">
												...
												<br/><b>returnUrl</b> :
												<br/><input  style="width:100%;" name="returnUrl" value="<%=siteDomain%>INIpayStdSample/INIStdPayReturn" >
												<!--
											...
											<div style="border:2px #dddddd double;padding:10px;background-color:#f3f3f3;">
												...
												<br/><input style="width:100%;" name="popupUrl" value="<%=siteDomain%>INIpayStdSample/popup" >
...
<script>
	...
	$(document).ready(function(){
		...
		/* --------- 자동 현재 날짜 ------------ */
		document.getElementById('date_ticketed').value= new Date().toISOString().slice(0, -1);
		/* ---------End 자동 현재 날짜 ------------ */

		/* ---------  좌석 번호 클릭이벤트 ------------  */
		$("#seat_num").click(function(){
			debugger;
				const checkSeat = $("#seat_num").val();
				$("#resultSeat").text(checkSeat);
...
/*  $("#testdata").click(function(){
	 debugger;
	 const user_id = '<input type="hidden" name="user_id" value='+id+' >';
	 debugger;
...
/* 		 	$.ajax({
			 	type: 'post',   //post방식으로 명시
				url : '/ticketing',  //이동할 controller url 
				dataType:'json',   //json형식으로 받기
				data : {
				"user_id": user_id.val(),
				"coupon_id": coupon_id.val(),
				"resultSeat":resultSeat.val(),
				"prfnm":prfnm.val(),
				"prfplcnm":prfplcnm.val(),
				"check_used":check_used.val(),
				},
				success: function(data){   //데이터 주고받기 성공했을 경우 실행할 결과
...
/* 	}); */
    ...
    <script src="resources/js/ticketing/jquery-3.3.1.min.js"></script>
    <script src="resources/js/ticketing/popper.min.js"></script>
    <script src="resources/js/ticketing/bootstrap.min.js"></script>
    <script src="resources/js/ticketing/rome.js"></script>

    <script src="resources/js/ticketing/main.js"></script>
    <script src="resources/js/ticketing/script.js"></script>
</body>
</html>